...
// Strategy: rename current .claude to .claude.bak.<name>, then
// copy profile dir to .claude (symlinks can break Claude Code on macOS).
export function activateClaudeProfile(profile) {
  const profileDir = profile.configDir;

  // Backup current .claude if it's NOT already a profile dir
  if (fs.existsSync(CLAUDE_DIR)) {
    const stat = fs.lstatSync(CLAUDE_DIR);
    if (stat.isDirectory()) {
      const backupName = `${CLAUDE_DIR}.aiswitch-backup`;
      if (!fs.existsSync(backupName)) {
        fs.renameSync(CLAUDE_DIR, backupName);
      } else {
        // Already backed up — just remove current
        fs.rmSync(CLAUDE_DIR, { recursive: true, force: true });
      }
    }
  }

  // Copy profile dir → ~/.claude  (or create empty if first time)
  if (fs.existsSync(profileDir) && fs.readdirSync(profileDir).length > 0) {
    execSync(`cp -r "${profileDir}/." "${CLAUDE_DIR}/"`, { stdio: 'pipe' });
  } else {
...
export function syncClaudeProfileBack(profileName) {
  ...
  if (fs.existsSync(CLAUDE_DIR)) {
    fs.mkdirSync(profileDir, { recursive: true });
    execSync(`rsync -a --delete "${CLAUDE_DIR}/" "${profileDir}/"`, { stdio: 'pipe' });
  }
}
